...
/**
 * IoTserver create DeviceReading asset.
 * @param {roll_royce.challenge.com.CreateDeviceReading} createDeviceReading
 * @transaction
 */
async function CreateDeviceReading(createDeviceReading) {
  let newDeviceReading;
  try {
    const factory = getFactory();
    return await getAssetRegistry('roll_royce.challenge.com.DeviceReading')
        .then(function(deviceReadingRegistry) {
          newDeviceReading = factory.newResource('roll_royce.challenge.com', 'DeviceReading', createDeviceReading.deviceID);
          newDeviceReading.deviceID = createDeviceReading.deviceID;
          newDeviceReading.hash = createDeviceReading.hash;
          newDeviceReading.initialDate = createDeviceReading.initialDate;
          newDeviceReading.endDate = createDeviceReading.endDate;
          return deviceReadingRegistry.add(newDeviceReading);
        })
        .then(function() {
          const event = factory.newEvent('roll_royce.challenge.com', 'CreateDeviceReadingEvent');
          event.deviceReading = newDeviceReading;
          emit(event);
        });
  } catch (e) {
    console.log(e);
